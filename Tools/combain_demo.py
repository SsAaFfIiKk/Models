...
if __name__ == '__main__':
    with open("video_analysis.data", "rb") as f:
    ...
    for i in range(len(intervals) - 1):
        ...
        """
        После первой итерации при обрабоки башки выкидывает эти ворнинги и начинает выдовать нули.
        Я отдельно запускал изначальный файл что ты мне кидал и там таже самая картина.
        RuntimeWarning: invalid value encountered in multiply lower_bound = _a * scale + loc
        RuntimeWarning: invalid value encountered in multiply upper_bound = _b * scale + loc
        RuntimeWarning: invalid value encountered in less head_mov_num += (dists < conf[0]).sum() + (dists > conf[1]).sum()
        RuntimeWarning: invalid value encountered in greater head_mov_num += (dists < conf[0]).sum() + (dists > conf[1]).sum()
        head_mov_30s_list содержит numpy.int32 на который ругаеться джейсон
        """
